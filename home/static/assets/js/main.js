...
const totalSlides = slides.length / 2; // Chúng ta chỉ tính 3 slide gốc

function showSlides() {
  slideIndex++;
  if (slideIndex >= totalSlides) {
    slideIndex = 0;
    slideshow.style.transition = "none"; // Tắt hiệu ứng chuyển tiếp khi quay lại slide đầu tiên
    slideshow.style.transform = `translateX(0%)`; // Quay lại đầu tiên ngay lập tức
    setTimeout(() => {
      slideshow.style.transition = "transform 1s ease"; // Kích hoạt lại hiệu ứng chuyển tiếp
    }, 0);
  } else {
    slideshow.style.transform = `translateX(${-slideIndex * 100}%)`;
  }
  setTimeout(showSlides, 3000); // Điều chỉnh tốc độ slideshow (3 giây)
}
